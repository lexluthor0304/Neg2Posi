...
def save_image(rgb, path):
    ...
    if clipped.ndim == 2:
        image_16 = np.round(clipped * 65535.0).astype(np.uint16)
        write_target = image_16
    else:
        image_16 = np.round(clipped * 65535.0).astype(np.uint16)
        write_target = cv2.cvtColor(image_16, cv2.COLOR_RGB2BGR)

    png_params = [cv2.IMWRITE_PNG_COMPRESSION, 2]
    success = cv2.imwrite(str(png_path), write_target, png_params)
    if not success:
        ...
        try:
            ...
            if clipped.ndim == 2:
                fallback = np.round(clipped * 65535.0).astype(np.uint16)
                PILImage.fromarray(fallback, mode="I;16").save(str(png_path))
            else:
                fallback = np.round(clipped * 255.0).astype(np.uint8)
                PILImage.fromarray(fallback, mode="RGB").save(str(png_path), compress_level=2)
